...
/* ─────────────────────────────────────────
   SHEETJS — CDN 동적 로드 (xlsx import 대신)
───────────────────────────────────────── */
let _xlsxReady = null;
function loadXLSX() {
...
async function parseTxnsExcel(arrayBuffer) {
  ...
  for (const row of rows) {
    ...
    const merchant = String(rawMerch).trim();
    result.push({ date, merchant, amount: amtNum, time, cancelled, source: "excel", key: `${date}|${merchant}|${amtNum}` });
  }
  return result;
}
...
───────────────────────────────────────── */
const CATS = [
  { id:"cafe",     label:"카페",      emoji:"☕", bg:"#FEF3C7", fg:"#92400E", kw:["빽다방","스타벅스","바나프레소","공차","쿠지","우지커피","쥬씨","커피","매머드커피","카페","coffee"] },
  { id:"bakery",   label:"베이커리",  emoji:"🍩", bg:"#FCE7F3", fg:"#9D174D", kw:["원도너츠","사롱메이드","태극당","크리스피크림","효자베이커리","떡의작품","베이글리","베이커리","bakery","uncle"] },
  { id:"delivery", label:"배달",      emoji:"🛵", bg:"#FEE2E2", fg:"#991B1B", kw:["우아한형제들","쿠팡이츠"] },
  { id:"grocery",  label:"마트",      emoji:"🛒", bg:"#D1FAE5", fg:"#065F46", kw:["이마트","웰빙마트","수지농협","로컬푸드","정남미","미트홈"] },
  { id:"shopping", label:"쇼핑",      emoji:"🛍️", bg:"#EDE9FE", fg:"#4C1D95", kw:["아성다이소","무인양품","신세계","스타필드","에스에스지","ssg","더블유컨셉","w컨셉","삼성전자"] },
  { id:"coupang",  label:"쿠팡",      emoji:"📦", bg:"#DBEAFE", fg:"#1E3A8A", kw:["쿠팡"] },
  { id:"transit",  label:"교통",      emoji:"🚇", bg:"#E0F2FE", fg:"#0C4A6E", kw:["티머니","카카오_택시","코레일","지하철","택시"] },
  { id:"sub",      label:"구독",      emoji:"💻", bg:"#EEF2FF", fg:"#3730A3", kw:["anthropic","openai","apple","구글페이먼트","google","누하스","네이버 파이낸셜"] },
  { id:"health",   label:"의료",      emoji:"🏥", bg:"#FDF2F8", fg:"#831843", kw:["피부과","의원","약국","메라키플레이스","병원"] },
  { id:"cvs",      label:"편의점",    emoji:"🏪", bg:"#F1F5F9", fg:"#334155", kw:["gs25","씨유","cu","지에스리테일","비지에프","세븐일레븐","미니스톱"] },
  { id:"travel",   label:"여행",      emoji:"✈️", bg:"#CCFBF1", fg:"#134E4A", kw:["제천킹스파","이천태극","치악","휴게소","풀무원","강릉사랑","코레일유통","호텔","숙소","펜션"] },
];
const FOOD_KW = ["김밥","냉면","비빔밥","막국수","만두","핫도그","떡볶이","맘스터치","맥도날드","하이꼬꼬","호앙비엣","오봉집","연희","청담동마녀","백남옥","무릎탁","써브웨이","subway","kfc","케이에프씨","돈꿈","갑부","케이콜라보","뱅뱅막국수","푸르파파","명랑","창고","어죽","식당","순대","곱창","삼겹","갈비","치킨","피자","라멘","라면","우동","초밥","스시","버거"];

function getCat(m) {
  const l = m.toLowerCase().replace(/[\s(주)주식회사농업회사법인㈜]/g, "");
  for (const c of CATS) if (c.kw.some(k => l.includes(k.toLowerCase().replace(/\s/,"")))) return c;
  if (FOOD_KW.some(k => l.includes(k))) return { id:"food", label:"식비", emoji:"🍽️", bg:"#FFF7ED", fg:"#7C2D12" };
...
function cleanM(m) {
  return m
    ...
    .replace(/\s*(강남우성점|수지구청역점|수지구청점|역삼점|죽전점|스타필드 수원점|서초우성직영점|서초뱅뱅점|뱅뱅사거리점|수지에이스점|강남역점|풍덕천3호점|옥수역점|용인수지DT점|판교유스|익스프레스)/g, "")
    .replace(/\s*SAN FRANCISCO USA/g, "")
    .trim().slice(0, 22);
}

/* ─────────────────────────────────────────
   스토리지 (window.storage — 아티팩트용)
   Vercel 배포 시 localStorage로 자동 전환
───────────────────────────────────────── */
...
function ImportModal({ onClose, onAdd }) {
  ...
  const handleFile = async e => {
    ...
    try {
      ...
      const rows = await parseTxnsExcel(ab);
      if (rows.length === 0) throw new Error("빈 결과");
      setExcelRows(rows);
    } catch (err) {
      console.error(err);
      setExcelFile(""); setExcelRows(null);
      alert("파일을 읽을 수 없어요.\n카드사 사이트에서 받은 .xls / .xlsx 파일인지 확인해주세요.");
    }
    setLoading(false);
  };

  const previewCount = mode === "text"
    ? parseTxnsText(rawText).length
    : (excelRows?.length ?? 0);

  const handleAdd = () => {
    const rows = mode === "text" ? parseTxnsText(rawText) : excelRows;
    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, display:"flex", alignItems:"flex-end", justifyContent:"center", background:"rgba(0,0,0,0.45)", backdropFilter:"blur(8px)" }} onClick={onClose}>
      <div style={{ background:"#fff", borderRadius:"28px 28px 0 0", width:"100%", maxWidth:580, maxHeight:"88vh", overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 -12px 60px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()} className="sheet-up">
        ...
        <div style={{ padding:"16px 24px 12px", borderBottom:"1px solid #F1F5F9", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:17, fontWeight:700, color:"#0F172A", letterSpacing:"-0.02em" }}>카드 내역 추가</div>
        ...
        <div style={{ padding:"12px 24px 24px", borderTop:"1px solid #F8FAFC", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <button onClick={handleAdd} disabled={previewCount === 0}
...
export default function App() {
  ...
  const [tab, setTab]           = useState(0);   // 0=홈 1=내역 2=분석
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#FAFAFA", fontFamily:"Manrope,'Apple SD Gothic Neo',sans-serif", color:"#0F172A", maxWidth:480, margin:"0 auto", position:"relative" }}>
        ...
        <div style={{ padding:"0 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:800, letterSpacing:"-0.04em", color:"#0F172A" }}>소비 리포트</div>
            <div style={{ fontSize:11, color:"#CBD5E1", marginTop:1 }}>로그인 없이 사용 가능</div>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={() => setImport(true)} style={IBN} title="내역 추가">＋</button>
            <button onClick={handleReset} style={IBN} title="초기화">↺</button>
          </div>
        </div>
              ...
              <div>
                {valid.length === 0 ? (
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"60px 0", gap:16 }}>
                    ...
                    <div style={{ fontSize:18, fontWeight:800, color:"#0F172A", letterSpacing:"-0.03em" }}>카드 내역이 없어요</div>
                    ...
                    <button onClick={() => setImport(true)}
                      style={{ marginTop:8, padding:"13px 32px", borderRadius:16, background:"#0F172A", color:"#fff", border:"none", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                      내역 추가하기
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <div style={{ marginBottom:28 }}>
                      ...
                      <div style={{ fontSize:42, fontWeight:800, color:"#0F172A", letterSpacing:"-0.04em", lineHeight:1.1, fontFamily:"'Instrument Serif',serif" }}>
                    ...
                    <div style={{ ...CS, marginBottom:14 }}>
                      ...
                      {catData.slice(0, 5).map((c, i) => (
                        <button key={i} onClick={() => openCat(c)}
                          ...
                          <div style={{ textAlign:"right", flexShrink:0 }}>
                            <div style={{ fontSize:14, fontWeight:700, color:"#0F172A", letterSpacing:"-0.02em" }}>{c.value.toLocaleString()}</div>
              ...
              <div>
                ...
                {grouped.length === 0 ? (
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"60px 0", gap:16 }}>
                    ...
                    <div style={{ fontSize:17, fontWeight:700, color:"#0F172A" }}>내역이 없어요</div>
              ...
              <div>
                {valid.length === 0 ? (
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"60px 0", gap:14 }}>
                    <div style={{ fontSize:56 }}>📊</div>
                    <div style={{ fontSize:17, fontWeight:700, color:"#0F172A" }}>분석할 데이터가 없어요</div>
                  </div>
                ) : (
                  <>
                    {/* 도넛 */}
                    ...
                    <div style={{ ...CS }}>
                      {catData.map((c, i) => (
                        <button key={i} onClick={() => openCat(c)}
                          style={{ display:"flex", alignItems:"center", gap:12, width:"100%", padding:"12px 0", background:"none", border:"none", cursor:"pointer", borderBottom:i<catData.length-1?"1px solid #F8FAFC":"none" }}>
